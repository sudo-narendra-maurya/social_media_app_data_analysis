...
WHO WON??!!*/
SELECT users.username, photos.id, photos.image_url, COUNT(likes.*) AS Total_Likes
FROM likes
...
SELECT ROUND((SELECT COUNT(*)FROM photos)/(SELECT COUNT(*) FROM users),2) AS "AVERAGE USER POST";
...
SELECT 
    ...
    (tableA.total_A::float / (SELECT COUNT(*) FROM users) * 100) AS "% who never commented",
    tableB.total_B AS "Number of Users who liked every photo",
    (tableB.total_B::float / (SELECT COUNT(*) FROM users) * 100) AS "% who liked every photo",
    (tableA.total_A + tableB.total_B)::float / (SELECT COUNT(*) FROM users) * 100 AS "% either never commented or liked every photo"
FROM
    (
        ...
        FROM
            (SELECT users.id
             ...
             LEFT JOIN comments ON users.id = comments.user_id
             GROUP BY users.id
             HAVING COUNT(comments.id) = 0) AS total_number_of_users_without_comments
    ) AS tableA,
    (
        SELECT COUNT(*) AS total_B
        FROM
            (SELECT users.id, COUNT(likes.photo_id) AS "total_likes_by_user"
             FROM users
             JOIN likes ON users.id = likes.user_id
             GROUP BY users.id
             HAVING COUNT(likes.photo_id) = (SELECT COUNT(*) FROM photos)) AS total_number_users_likes_every_photos
    ) AS tableB;
...
SELECT 
    tableA.total_A AS "Number Of Users who never commented",
    (tableA.total_A::float / (SELECT COUNT(*) FROM users) * 100) AS "%",
    tableB.total_B AS "Number of Users who commented on photos",
    (tableB.total_B::float / (SELECT COUNT(*) FROM users) * 100) AS "%"
FROM
    (
        SELECT COUNT(*) AS total_A 
        FROM
    ...
    (
        ...
        FROM
            (SELECT users.id
             ...
             LEFT JOIN comments ON users.id = comments.user_id
             GROUP BY users.id
             HAVING COUNT(comments.id) > 0) AS total_number_users_with_comments
    ) AS tableB;
